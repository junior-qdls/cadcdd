...
def group_places_by_distance(total_days, places):
    ...
    # kmeans processing
    kmeans = EqualGroupsKMeans(n_clusters=total_days, random_state=0).fit(
        np_array
    )
...
class EqualGroupsKMeans(BaseEstimator, ClusterMixin, TransformerMixin):
    ...
    def _check_test_data(self, X):
        X = check_array(
            X, accept_sparse="csr", dtype=FLOAT_DTYPES, warn_on_dtype=True
        )
...
def _kmeans_single(
    ...
    best_labels, best_inertia, best_centers = None, None, None
    # init
    centers = k_means_._init_centroids(
        X,
        n_clusters,
        init,
        random_state=random_state,
...
def _labels_inertia_precompute_dense(X, x_squared_norms, centers, distances):
    ...
    labels, mindist = initial_assignment(
        labels, mindist, n_samples, all_distances, max_cluster_size
    )
    ...
    for point in points_by_high_distance:
        ...
        cluster_id, point_dist = get_best_cluster_for_point(
            point, all_distances
        )
        if (
            not is_cluster_full(cluster_id, max_cluster_size, labels)
            and point_cluster != cluster_id
        ):
            labels[point] = cluster_id
            mindist[point] = point_dist
            best_labels = labels.copy()
        ...
        for swap_candidate in transfer_list:
            ...
            if point_cluster != cand_cluster:
                ...
                if point_distance < cand_distance:
                    ...
                    mindist[swap_candidate] = all_distances[
                        point_cluster, swap_candidate
                    ]

                    if (
                        np.absolute(mindist).sum()
                        < np.absolute(best_mindist).sum()
                    ):
...
def sort_adjust_row(points_distances):
    ...
    return sorted(
        [
            (cluster_id, point_dist)
            for cluster_id, point_dist in enumerate(points_distances)
        ],
        key=lambda x: x[1],
    )
